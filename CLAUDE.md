...
2. **整個 pipeline 在 Linux GPU 機跑**（RTX 5090, CUDA 13）。Mac MPS path 在 spec 裡規劃過但**從未實測**；如果 Kojie 之後要在本機跑，需要重新驗證 mlx-whisper / demucs-mlx / pyannote MPS 行為。
...
- **Kojie 已經很熟、知道原唱怎麼唱的歌**（音高 / 對齊出錯時抓得到）
- 4 分鐘以內（短一點 debug 快）
...
## 跨專案 context
- Kojie 的 Mac：Apple Silicon（**目前 pipeline 沒在 Mac 跑過**）
- 主 dev / runtime 機：`ntnumaplab2`（RTX 5090, 33.67 GB VRAM, MeanAudio 訓練那台）
