...
DEFAULT_DATASET = Path("/home/kojiek/side_projects/itako/itako_singing")
DEFAULT_SKIP_INPUT = {"pau", "br", "sil"}
DEFAULT_IGNORE_EVAL = {"", "pau", "br", "sil", "SP", "AP", "<SP>", "<AP>"}
...
def read_kiritan_lab(path: Path, *, skip: set[str] | None = None) -> list[PhoneInterval]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if ph in skip:
            continue
        # Itako mono_label is HTK 100ns ticks; Kiritan's is decimal seconds.
        if ("." in s) or ("." in e):
...
def prepare_cmd(dataset: str, out_dir: str, limit: int, sofa_tokenization: str) -> None:
    ...
    click.echo(f"[itako-phone] prepared {len(labs)} songs -> {out}")
