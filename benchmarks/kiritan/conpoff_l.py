#!/usr/bin/env python3
"""COnPOff+L — lyric-conditioned note transcription metric (first implementation).

A predicted note counts as correct only if ALL FOUR conditions pass:
  onset  |ref-est| <= 50 ms
  pitch  |ref-est| <= 50 cents
  offset |ref-est| <= max(50 ms, 0.2 x ref duration)
  lyric  romaji(mora) label of est onset == romaji label of ref onset

Matching machinery follows mir_eval.transcription (candidate pairs ->
maximum bipartite matching), the same shape as mir_eval.transcription_velocity
extends the family with a 4th per-note condition (velocity there, lyric here).

Mora attribution (the "loose" karaoke reading): each note onset is owned by
the mora whose ownership interval [mora_onset_i, mora_onset_{i+1}) contains it.
`cl` (geminate closure) merges into the FOLLOWING mora (notes never carry っ);
`N` stays a standalone singable mora. GT morae come from mono_label; predicted
morae come from the MMS forced-alignment .lab (same phone sequence, different
times — so L isolates TIMING attribution, the karaoke "right bar, right word"
question, with lyrics known).

Outputs the ladder COn -> COnP -> COnPOff -> COnPOff+L per (note model x
aligner), P(L | COnPOff-matched), and paired bootstrap CIs over songs.

    ~/venvs/karaoke-jp/bin/python benchmarks/kiritan/conpoff_l.py
"""
...
def read_lab(path: Path) -> list[tuple[float, float, str]]:
    ...
    for line in path.read_text().splitlines():
        ...
        if len(parts) != 3:
            continue
        s, e, ph = parts
        rows.append((float(s), float(e), ph))
    return rows
...
def main() -> None:
    ...
    for nm_name, nm in note_models.items():
        for al_name, al_dir in aligners.items():
            per_song = []
            for s in songs:
                est_morae = gt_morae[s] if al_dir is None else group_morae(read_lab(al_dir / f"{s}.lab"))
                if al_dir is not None:
                    g_seq = [m[1] for m in gt_morae[s]]
                    e_seq = [m[1] for m in est_morae]
                    if g_seq != e_seq:
                        print(f"  ! {s}: mora sequence mismatch GT {len(g_seq)} vs {al_name} {len(e_seq)}")
                per_song.append(eval_song(gt[s], nm[s], gt_morae[s], est_morae))
            results[(nm_name, al_name)] = per_song

    print("=== COnPOff+L ladder (Kiritan N=50, gt_timefix, macro per-song F1) ===")
    ...
    print("\n=== Discrimination test (paired bootstrap over 50 songs, 95% CI) ===")
    pairs = [
    ...
    print("\n=== Tax uniformity (COnPOff -> COnPOff+L drop per system) ===")
    for (nm_name, al_name), per_song in results.items():
    ...
    out = {f"{nm}x{al}": {
        ...
        "COnPOff": round(macro(ps, "COnPOff"), 4), "COnPOff+L": round(macro(ps, "COnPOff+L"), 4),
        "P_L_given_match": round(sum(s["n_lyric_ok"] for s in ps) / max(1, sum(s["n_matched"] for s in ps)), 4),
    } for (nm, al), ps in results.items()}
    (HERE / "conpoff_l_results.json").write_text(json.dumps(out, indent=2))
