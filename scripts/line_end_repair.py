...
lines, mega-segments), but leaves a residual **systematic early cutoff**: line
ends land ~0.5-1.9 s before the true vocal offset.  Cause: ``midi_timing`` pins the
...
def voiced_tail_end(
    ...
    *,
    ...
    ``decay_db`` (relative-decay mode): when > 0, a frame
...
@click.option("--decay-db", default=0.0, show_default=True,
              help="Relative-decay tail: also follow frames "
