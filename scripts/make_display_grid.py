#!/usr/bin/env python3
"""Standardized display grid for the pitch bars.
...
Fixed visual vocabulary:
  ...
  * a note's SLOT is exactly its quantized duration; the bar is drawn
    ...
    cursor speed across note values (drawing the gap outside the slot makes
    4th+gap vs 2nd+gap not 2:1, so the cursor would wobble even on tempo)
...
# melisma notes on one mora closer than this (real s) get a ligature stroke
LIGATURE_MAX_GAP = 0.15
...
def drop_fragments(notes: list[Note], *, min_note: float) -> tuple[list[Note], int]:
    ...
    almost always a transcription split of the same sung note.
    """
...
WIGGLE_MODES = ("cascade", "static", "noalt", "off")


def absorb_wiggles(
    notes: list[Note], *, max_dur: float = 0.22, mode: str = "cascade",
) -> tuple[list[Note], int]:
    ...
    mode:
      cascade -- relabels in place, so each relabel feeds the next test.
                 An alternating figure (A G A G A) collapses to all-A.
      static  -- every decision reads the ORIGINAL pitches (no chaining).
      noalt   -- static, and additionally keeps b when its pitch recurs two
                 notes away (i±2): that is a sung alternation, not a wiggle.
      off     -- no relabelling.
    """
    if mode not in WIGGLE_MODES:
        raise ValueError(f"unknown wiggle mode {mode!r}")
    out = [list(n) for n in sorted(notes)]
    if mode == "off":
        return [tuple(n) for n in out], 0
    ref = out if mode == "cascade" else [list(n) for n in out]
    fixed = 0
    for i in range(1, len(out) - 1):
        a, b, c = ref[i - 1], ref[i], ref[i + 1]
        if (
            b[1] - b[0] <= max_dur
            and a[2] == c[2]
            and b[2] != a[2]
            ...
            if mode == "noalt" and (
                (i >= 2 and ref[i - 2][2] == b[2])
                or (i + 2 < len(ref) and ref[i + 2][2] == b[2])
            ):
                continue
            out[i][2] = a[2]
            fixed += 1
...
def apply_pitch_patch(
    ...
    """Hand-written per-song pitch fixes, display layer ONLY.
...
def _char_windows(aligned: list[dict], *, pad: float = 0.35) -> list[tuple[float, float]]:
    ...
    The RMS VAD misses soft/breathy passages (e.g. a whispered scat hook),
    ...
    in an interlude) without killing softly sung notes.
    Shared with make_portrait_grid.
    """
    spans = sorted(
...
def layout_pages(
    ...
    *,
    ...
    Slot semantics: each note occupies exactly its quantized duration;
...
@click.option("--gap-units", type=float, default=0.0625, show_default=True,
              help="Fixed gap between morae, in quarters (just visibly "
              "separate; 0.25 and 0.125 read as too sparse).")
...
@click.option("--wiggle-mode", type=click.Choice(WIGGLE_MODES), default="cascade",
              show_default=True, help="absorb_wiggles variant (see its docstring).")
...
@click.option("--pitch-patch", "pitch_patch_path",
              ...
              help="JSON list of hand-written pitch fixes, display layer only. "
              "See apply_pitch_patch().")
...
@click.option("--mora-cleanup", type=bool, default=False, show_default=True,
              help="One bar per mora (needs --aligned): cut fused legato notes "
              "at MMS char onsets, then note_cleanup's mora-aware pass (same-"
              "pitch shatter / scoop merges, <=3 st in-mora variants -> one "
              "pitch, acoustically-gated phantom drops). Never merges ACROSS "
              "morae, so sung alternations survive (cf. wiggle_mode off).")
@click.option("--out-ligatures", "ligatures_path", type=click.Path(dir_okay=False),
              default=None, help="EXPERIMENTAL (needs --aligned): write the "
              "display-time pairs of consecutive different-pitch notes sung on "
              "ONE mora (melisma), for render_mp4 --ligatures to join them "
              "with a connecting stroke (連筆).")
@click.option("--vocals", "vocals_path", type=click.Path(exists=True, dir_okay=False),
              default=None, help="vocals.wav — with --rmvpe-f0/--pyin-f0 arms "
              "the acoustic rules of --mora-cleanup.")
@click.option("--rmvpe-f0", "rmvpe_path", type=click.Path(exists=True, dir_okay=False), default=None)
@click.option("--pyin-f0", "pyin_path", type=click.Path(exists=True, dir_okay=False), default=None)
...
def main(midi_path, bpm_file, out_midi, out_warp, quarters_per_page,
         ...
         breath_gap, breath_units, rms_segments_path, wiggle_mode,
         mora_cleanup, vocals_path, rmvpe_path, pyin_path, ligatures_path):
    ...
    if aligned_path:
        ...
        if rms_segments_path:
            # gate to lyric windows ∩ (RMS voiced ∪ MMS char evidence). A bare
            # RMS intersection would cut softly-sung passages the VAD misses but
            # the CTC aligner heard; the char-evidence union keeps interlude
            # ghost protection (no chars in an interlude) while sparing them.
    ...
    notes, wiggles = absorb_wiggles(notes, mode=wiggle_mode)

    mora_split = 0
    cstats: dict = {}
    if mora_cleanup:
        if not aligned_path:
            raise click.UsageError("--mora-cleanup needs --aligned")
        sys.path.insert(0, str(ROOT / "scripts"))
        import note_cleanup as nc

        aligned_list = json.loads(Path(aligned_path).read_text(encoding="utf-8"))
        # after drop/absorb, or the slivers fold straight back in
        notes, mora_split = nc.split_notes_at_chars(notes, aligned_list)
        ev = nc.AcousticEvidence.load(vocals_path, rmvpe_path, pyin_path)
        if not ev:
            click.echo("[display-grid] mora-cleanup without --vocals/--rmvpe-f0/"
                       "--pyin-f0: merges only, acoustic drops skipped")
        notes, cstats, clog = nc.cleanup(notes, aligned_list, ev)
        for msg in clog:
    ...
    if pitch_patch_path:
        ...
        # Hand corrections: melisma split + note drops.
    ...
    if ligatures_path:
        if not aligned_path:
            raise click.UsageError("--out-ligatures needs --aligned")
        sys.path.insert(0, str(ROOT / "scripts"))
        import note_cleanup as nc

        moras, _slots = nc.assign_true_moras(
            notes, json.loads(Path(aligned_path).read_text(encoding="utf-8")))
        # layout walks pages -> phrases -> note indices in order
        order = [i for page in pages for ph in page for i in ph]
        disp_of = {i: disp_notes[k] for k, i in enumerate(order)}
        pairs = []
        for i in range(len(notes) - 1):
            a, b = notes[i], notes[i + 1]
            if (moras[i] != -1 and moras[i] == moras[i + 1] and a[2] != b[2]
                    and b[0] - a[1] <= LIGATURE_MAX_GAP
                    and i in disp_of and i + 1 in disp_of):
                pairs.append([round(disp_of[i][0], 4), round(disp_of[i + 1][0], 4)])
        Path(ligatures_path).write_text(json.dumps({"pairs": pairs}))
        click.echo(f"[display-grid] ligatures={len(pairs)} -> {ligatures_path}")
    ...
    click.echo(f"[display-grid] notes={len(disp_notes)} pages={len(pages)} "
               ...
               f"mora_split={mora_split} cleanup={cstats} "
