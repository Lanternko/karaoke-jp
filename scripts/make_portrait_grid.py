...
# single source of truth (used by both grids' RMS∪char gating).
...
def drop_isolated_unvoiced(
    ...
    *,
    ...
    The CTC stretches a once-written scat line
    ...
    Caveat: a wholly-soft long phrase (RMS-unvoiced throughout) could be
    over-dropped — `ghosts` is logged so a portrait render surfaces it.
    """
...
# split_notes_at_chars moved to note_cleanup (shared with the horizontal grid)
split_notes_at_chars = nc.split_notes_at_chars


# apply_melisma_splits / apply_note_drops now live in make_display_grid (mdg)
# as the single source of truth for both grids. Portrait calls mdg.apply_melisma_splits / mdg.apply_note_drops.
...
def build_bar_lines(
    ...
    *,
    ...
    This matches reading order and two break rules: (1) the row is
...
def apply_lyric_retime(lines: list[dict], patches: list[dict]) -> int:
    ...
    the one lyric line that IS there across the whole region (its trailing
    syllables can end up matched to interlude percussion). An override {"lyric_retime":[lo,hi],"to":[t0,t1]}
...
@click.option("--wiggle-mode", type=click.Choice(mdg.WIGGLE_MODES), default="cascade",
              show_default=True, help="absorb_wiggles variant (make_display_grid).")
...
def main(midi_path, warp_path, bpm_file, aligned_path, quarters_per_row,
         ...
         pyin_path, backing_path, out_path, wiggle_mode):
    ...
    # from it: the raw collapsed alignment (a char spanning a long
    ...
    notes, wiggles = mdg.absorb_wiggles(notes, mode=wiggle_mode)
