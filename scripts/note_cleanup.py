#!/usr/bin/env python3
"""Mora-aware display-note cleanup.

GAME segments acoustically, so three families of display bugs survive the
existing drop_fragments/absorb_wiggles pass:

  1. SCOOP mis-splits — a しゃくり approach (sing a step low, slide up into
     the sustain) becomes two notes on ONE mora (e.g. 65 then 67).
  2. same-pitch shatter — one held note splits into long + short pieces a
     few tens of ms apart (e.g. 67 + 67 with a 20 ms gap).
  3. phantom tails — reverb/echo in the separated stem after the voice
     stops transcribes as short fragments (the stem is -30..-60 dB and
     neither f0 tracker supports the note's pitch).
...
`f0 > 0` from one tracker alone is NOT trusted (RMVPE can report pitch in
dead-silent interludes and jump an octave on breathy passages); a note survives only with stem energy AND a tracker at its pitch.
...
# (real descending melismas are protected by the ascending-only gate)
SCOOP_MAX_STEP = 3
SCOOP_MIN_RATIO = 1.5
SCOOP_MAX_GAP = 0.04
...
ORPHAN_ABS_DB = -40.0    # orphan floor (silent-interlude ghosts sit far below; soft singing ~-26)
...
def apply_recut_to_aligned(aligned: list[dict], patches: list[dict]) -> int:
    ...
    reading the raw collapsed alignment — a char stretched across a long
    instrumental would bless ghost notes inside it.
...
def assign_moras(notes: list[Note], chars: list[Span]) -> list[int]:
    ...
    blurred adjacent morae into ties and mis-assigned scoops to the
    preceding mora. The pad only rescues notes that touch no window at
...
def _partition_char_notes(notes: list[Note], indices: list[int], n_morae: int) -> list[list[int]]:
    ...
    Equal time splitting is wrong for sung kanji (e.g. 静: し is short,
...
class AcousticEvidence:
    ...
    One tracker is never enough: RMVPE can report f0>0 inside a dead-silent
    interlude and read a breathy passage an octave up where pYIN holds the
    right pitch. Energy alone also fails: reverb
...
def fix_octave_errors(
    ...
    sustains a subharmonic octave DOWN, and the trust hierarchy is transcription >> any single
    ...
    for idx, (s, e, p) in enumerate(notes):
        ...
        if 0 < idx < len(notes) - 1:
            ...
            # range.  Repair it when shifting -12 makes it
...
def merge_scoops(
    ...
    that is what keeps real melismas like 69-67-67-69 intact.
    """
...
def consolidate_mora_primary(
    ...
    instead of extending the bar.  Changes wider
    ...
    raw duration alone can pick a slide over the sung attack plateau by a
    few tens of ms. The plateau the trackers actually hold is the score note; a same-length
...
def drop_unsupported_tails(
    ...
    uncharted ad-lib still must not paint a karaoke bar — e.g. an outro where
    accompaniment is strong enough to fool BOTH trackers after the final
    lyric has ended.
