...
def apply_manual_breaks(aligned: list[dict], breaks: list[str]) -> list[dict]:
    """Hand line breaks: each entry is a whole lyric line with ``|`` at the
    cut(s), e.g. ``"春の日差しの中で|君と歩いた道"`` (whitespace
    ignored). Cuts must fall on token boundaries. Unmatched entries raise, so a
    lyrics edit can never silently orphan a break."""
    def norm(t: str) -> str:
        return "".join(t.split())

    todo = {norm(b.replace("|", "")): norm(b) for b in breaks}
    out: list[dict] = []
    for line in aligned:
        spec = todo.pop(norm(line["text"]), None)
        if spec is None:
            out.append(line)
            continue
        cuts, n = set(), 0
        ...
        for tok in line["tokens"]:
            if n in cuts and chunks[-1]:
                chunks.append([])
            chunks[-1].append(tok)
            n += len(norm(tok.get("surface") or ""))
        if len(chunks) != len(cuts) + 1:
            raise ValueError(f"manual break not on a token boundary: {spec!r}")
        for ch in chunks:
            sung = [t for t in ch if t.get("chars")]
            out.append({**line, "manual_break": True,
                        "text": "".join(t["surface"] for t in ch).strip(),
                        "start": sung[0]["start"] if sung else line["start"],
                        "end": max((t["end"] for t in sung), default=line["end"]),
                        "tokens": ch})
    if todo:
        raise ValueError(f"manual breaks matched no lyric line: {list(todo.values())}")
    return out


_BOUND_POS = {"助詞", "助動詞", "接尾辞", "補助記号", "記号"}


def split_lines_at_pages(
    aligned: list[dict], page_starts: list[float], *, min_chars: int = 4
) -> list[dict]:
    """Cut lyric lines where the pitch-bar grid flips pages (horizontal only).

    ``page_starts`` are the REAL times (seconds) each display page takes over,
    ascending. A line whose tokens land on several pages is split at the token
    boundary where the page changes, so a lyric row never runs wider than the
    bars above it. Cuts only fall between tokens (never mid-word); a chunk with
    fewer than ``min_chars`` sung chars rides along with the neighbour it is
    closer to in time instead of becoming its own row.
    """
    import bisect

    out: list[dict] = []
    for line in aligned:
        toks = [t for t in line["tokens"] if t.get("chars")]
        # a hand-broken line is final: the author already chose its cuts
        if not toks or len(page_starts) < 2 or line.get("manual_break"):
            out.append(line)
            continue
        chunks: list[list[dict]] = []
        last_page = None
        for tok in toks:
            # first char's MIDPOINT: MMS char starts often lead the GAME note
            # onset by a few cs, which would drag a page's first word back
            # onto the previous page
            c0 = tok["chars"][0]
            page = bisect.bisect_right(page_starts, (c0["start"] + c0["end"]) / 2) - 1
            if page != last_page:
                chunks.append([])
                last_page = page
            chunks[-1].append(tok)

        # a row never starts with a word that leans on the previous one
        # (青く|て, 待ち続け|てん): particles / auxiliaries / suffixes / punctuation
        # ride back to the previous chunk even if their note is past the flip
        for k in range(1, len(chunks)):
            while chunks[k] and (chunks[k][0].get("is_punct")
                                 or chunks[k][0].get("pos") in _BOUND_POS):
                chunks[k - 1].append(chunks[k].pop(0))
        chunks = [ch for ch in chunks if ch]

        def n_chars(ch: list[dict]) -> int:
            return sum(len(t["chars"]) for t in ch if not t.get("is_punct"))

        while len(chunks) > 1:
            small = [i for i, ch in enumerate(chunks) if n_chars(ch) < min_chars]
            if not small:
                break
            i = small[0]
            gap_prev = (chunks[i][0]["start"] - chunks[i - 1][-1]["end"]) if i > 0 else float("inf")
            gap_next = (chunks[i + 1][0]["start"] - chunks[i][-1]["end"]) if i + 1 < len(chunks) else float("inf")
            j = i - 1 if gap_prev <= gap_next else i + 1
            lo, hi = min(i, j), max(i, j)
            chunks[lo:hi + 1] = [chunks[lo] + chunks[hi]]
        ...
        for ch in chunks:
            out.append({**line,
                        "text": "".join(t["surface"] for t in ch).strip(),
                        "start": ch[0]["start"],
                        "end": max(t["end"] for t in ch),
                        "tokens": ch})
    return out


def export_lrc(aligned_path: Path, out_path: Path, *, block_size: int = 2,
               page_starts: list[float] | None = None,
               manual_breaks: list[str] | None = None) -> None:
    ...
    if manual_breaks:
        aligned = apply_manual_breaks(aligned, manual_breaks)
    if page_starts:
        aligned = split_lines_at_pages(aligned, page_starts)
