...
@pytest.mark.parametrize(
    ...
    [
        "冷たい風に　揺れる影",
        "夜空を見上げて　ゆらりゆらり",  # legitimate 2-gram repeat (×2)
        "もっと　もっと　そばにいさせて",  # legitimate word repeat (×2)
...
def test_long_chorus_with_repeat_kept() -> None:
    # Realistic chorus: yurari-yurari + line follow-on, ~20 chars total.
    text = "夜空を見上げてゆらりゆらり胸を満たしては溢れ"
    assert not is_hallucinated_segment(text)
...
@pytest.mark.parametrize(
    "text",
    [
        "ご視聴ありがとうございました",  # emitted on a breathy ad-lib blip
        "ご視聴ありがとうございます",
        "チャンネル登録お願いします",
        "コメント欄で教えてください",
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "登録した名前を探して",  # 登録 alone isn't チャンネル登録
    ],
)
