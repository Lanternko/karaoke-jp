"""note_cleanup: the three display-bug families + their guard rails.

Every scenario is a minimal synthetic case: scoop mis-split, same-pitch
shatter, phantom tails/orphans, RMVPE octave-up outro, plus the guards — a real descending
...
# ---------- failure mode 1: scoop mis-split (65+67 -> 67) ----------
...
def test_mora_primary_prefers_tracker_supported_plateau():
    # く : attack plateau 70 (0.39s, both trackers pinned)
...
# ---------- failure mode 2: same-pitch shatter (67|67, 20ms gap) --------
...
def test_supported_soft_tail_survives():
    # ら outro echoes : quiet but BOTH trackers agree
...
def test_dead_silent_orphan_drops_despite_tracker_blip():
    # RMVPE reports 57 inside a -89 dB interlude — f0>0 alone
...
# ---------- octave fix (81 -> 69) and its guard ----------
...
def test_pyin_subharmonic_does_not_repitch_midrange():
    # GAME+RMVPE say 65, pYIN reads 53 (subharmonic).
...
def test_local_octave_island_repitches_inside_global_range():
    # the bad 70 is not a global high outlier, but
...
def test_different_pitch_spill_mostly_after_mora_drops_without_extending():
    # the real 65 plateau ends at 2.20; a 68 event
...
def test_assign_moras_unpadded_overlap_wins():
    # の: the scoop note must belong to の, not tie-break into た
